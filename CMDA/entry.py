#== service_func__   used by universalPlotting6b, 3b
#== def_generalServiceResp  by Chenxing  used by joinEOF and displayHonrizontalMean
...
#== universalPlotting4__
#== universalPlotting6__
...
#== timeSeriesWF_    
#== timeSeries__
#== twoDimZonalMean__    https start from here
#== threeDimZonalMean__
#== zonalMean__     -- called by zonalMean8.html
#== fileUpload__   not used
#== universalP3
#== fileUpload2__
#== fileUpload2a__
#== fileCheck__
...
from svc.src.universalPlotting4 import call_universalPlotting4
from svc.src.universalPlotting6 import call_universalPlotting6
...
# port number 9075 for summer school 2017 
...
def serviceFunc(service):
    ...
    import urllib
    urlLink = urlLink.strip() + '&plotUrl=%s&dataUrl=%s' % (urllib.quote(plotUrl), urllib.quote(dataUrl))
...
def generalServiceResp (serviceObj):
    ...
    try:
      ...
      (message, imgFileName, dataFileName) = serviceObj.processRequest()
      # chdir back
      os.chdir(current_dir)
      hostname, port = get_host_port2("host.cfg")
      ### userId = 2
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl(serviceObj.name, tag, imgFileName, dataFileName)
...
'''%(dirN2, dataFileName, var)
      ...
      hosthome1 = 'api.jpl-cmda.org'
      backend_url = 'https://' + hostname1 + '/svc/twoDimMap'
      print 'backend_url: ', backend_url
      plotUrl = 'https://' + hostname1 + '/static/twoDimMap/' + tag + '/' + imgFileName
      print 'plotUrl: ', plotUrl
      dataUrl = 'https://' + hostname1 + '/static/twoDimMap/' + tag + '/' + dataFileName
      print 'dataUrl: ', dataUrl
      failedImgUrl = 'https://' + hostname1 + '/static/plottingFailed.png'
      print 'failedImgUrl: ', failedImgUrl
...
def display_mapView():
    ...
    try:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('mapView', tag, imgFileName, dataFileName)
...
#       parameters_json[thisKey] = request.args.get(thisKey, '')
    ...
    try:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('timeSeries', tag, imgFileName, dataFileName)
...
def display_timeSeriesWorkFlow():
    ...
    try:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('timeSeriesWorkFlow', tag, imgFileName, dataFileName)
...
def displayTwoDimSlice3D():
    ...
    try:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('twoDimSlice3D', tag, imgFileName, dataFileName)
      # hostname1 = "api.jpl-cmda.org"
      # backend_url = 'https://' + hostname1 + '/svc/twoDimSlice3D'
      # print 'backend_url: ', backend_url
      # plotUrl = 'https://' + hostname1 + '/static/twoDimSlice3D/' + tag + '/' + imgFileName
      # print 'plotUrl: ', plotUrl
      # dataUrl = 'https://' + hostname1 + '/static/twoDimSlice3D/' + tag + '/' + dataFileName
      # print 'dataUrl: ', dataUrl
      # failedImgUrl = 'https://' + hostname1 + '/static/plottingFailed.png'
      # print 'failedImgUrl: ', failedImgUrl
...
def displayTwoDimZonalMean():
    ...
    try:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('twoDimZonalMean', tag, imgFileName, dataFileName)
...
#== fileUpload2a__
@app.route("/fileUpload2a", methods=["GET", "POST"])
@crossdomain(origin='*')
def upload2a():
  uploadDir = '/home/svc/upload'
  ALLOWED_EXTENSIONS = set(['nc', 'cdf']) 
  ...
  if request.method=="POST":
    ...
    if 1:
      try:
        ok1 = checkNc2.checkNc(destination, dict1, allowOverwrite=0)
      except Exception as e:
        dict1['success'] = False
        dict1['message'] += "file check failed. \n" + str(e)
        ok1 = 99
...
def fileCheck0():
    ...
    if request.method == 'GET':
        ...
        if fn1.find('static') > 0:
          temp1 = fn1.find('static')
          fn1 = '/home/svc/new_github/CMDA/JPL_CMDA/services/svc/svc/' + fn1File[temp1:]

        # to undo the javascript escape()
        import urllib
        fileName2 = urllib.unquote( str(fn1) )
        print fileName2
        dict1['fileName'] = fileName2
        dict1['source'] = 'online'
        try:
          ok1 = checkNc2.checkNc(fileName2, dict1, allowOverwrite=0)
          dict1['success'] = True
          print 'ok1: ', ok1
        except Exception as e:
          dict1['success'] = False
...
def displayThreeDimVerticalProfile():
    ...
    try:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('threeDimVerticalProfile', tag, imgFileName, dataFileName)
...
#    """Run displayUniversalPlotting3b"""
#    executionStartTime = int(time.time())
#    # status and message
#    success = True
#    message = "ok"
...
#== universalPlotting4__
@app.route('/svc/universalPlotting4', methods=["GET"])
@crossdomain(origin='*')
def displayUniversalPlotting4():
    """Run displayUniversalPlotting4"""
    executionStartTime = int(time.time())
    # status and message
    success = True
    message = "ok"
    ...
    #try:
      seed_str = str(time.time())
      tag = md5.new(seed_str).hexdigest()
      output_dir = current_dir + '/svc/static/universalPlotting4/' + tag
      print 'output_dir: ', output_dir
      if not os.path.exists(output_dir):
        os.makedirs(output_dir)
      json2['outDir'] = output_dir
      ...
      # chdir to where the app is
      os.chdir(current_dir+'/svc/src/universalPlotting4')
      # instantiate the app. class
      c1 = call_universalPlotting.call_universalPlotting(pFile)
      # call the app. function (0 means the image created is scatter plot)
      ### (message, imgFileName) = c1.displayScatterPlot2V(0)
      ...
      backend_url = 'http://' + hostname + ':' + port + '/svc/universalPlotting4'
      print 'backend_url: ', backend_url
      print 'imgFileName: ', imgFileName
      plotUrl = 'http://' + hostname + ':' + port + '/static/universalPlotting4/' + tag + '/' + imgFileName
      print 'plotUrl: ', plotUrl
      dataUrl = 'http://' + hostname + ':' + port + '/static/universalPlotting4/' + tag + '/' + dataFileName
      print 'dataUrl: ', dataUrl
      failedImgUrl = 'http://' + hostname + ':' + port + '/static/plottingFailed.png'
      print 'failedImgUrl: ', failedImgUrl
...
#== universalPlotting6__
@app.route('/svc/universalPlotting6', methods=["GET"])
@crossdomain(origin='*')
def displayUniversalPlotting6():
    """Run displayUniversalPlotting6"""
    executionStartTime = int(time.time())
    # status and message
    success = True
    message = "ok"
    ...
    #try:
      seed_str = str(time.time())
      tag = md5.new(seed_str).hexdigest()
      output_dir = current_dir + '/svc/static/universalPlotting6/' + tag
      print 'output_dir: ', output_dir
      if not os.path.exists(output_dir):
        os.makedirs(output_dir)
      json2['outDir'] = output_dir
      ...
      # chdir to where the app is
      os.chdir(current_dir+'/svc/src/universalPlotting6')
      # instantiate the app. class
      c1 = call_universalPlotting6.call_universalPlotting6(pFile)
      # call the app. function (0 means the image created is scatter plot)
      ### (message, imgFileName) = c1.displayScatterPlot2V(0)
      (message, imgFileName, dataFileName) = c1.display()
      # chdir back
      ...
      backend_url = 'http://' + hostname + ':' + port + '/svc/universalPlotting6'
      print 'backend_url: ', backend_url
      print 'imgFileName: ', imgFileName
      plotUrl = 'http://' + hostname + ':' + port + '/static/universalPlotting6/' + tag + '/' + imgFileName
      print 'plotUrl: ', plotUrl
      dataUrl = 'http://' + hostname + ':' + port + '/static/universalPlotting6/' + tag + '/' + dataFileName
      print 'dataUrl: ', dataUrl
      failedImgUrl = 'http://' + hostname + ':' + port + '/static/plottingFailed.png'
      print 'failedImgUrl: ', failedImgUrl
    ...
    if wfId:
      wfFile = current_dir + '/svc/static/WF/' + wfId
      print 'wfFile: ', wfFile
      if not os.path.exists(wfFile):
        temp2 = 'touch %s'%wfFile
        os.system(temp2)

      temp1 = '"%s": "%s",\n'%(json2['service'], plotUrl)
      fId = open(wfFile, 'r+')
      fid.write(temp1)
      fid.close()
...
#    print("Run displayUniversalPlotting6b")
#    executionStartTime = int(time.time())
#    # status and message
#    success = True
#    message = "ok"
...
#      seed_str = str(time.time())
#      tag = md5.new(seed_str).hexdigest()
#      output_dir = current_dir + '/svc/static/universalPlotting6b/' + tag
#      print 'output_dir: ', output_dir
#      if not os.path.exists(output_dir):
#        os.makedirs(output_dir)
#      json2['outDir'] = output_dir
...
#
#      # chdir to where the app is
#      os.chdir(current_dir+'/svc/src/universalPlotting6b')
#      # instantiate the app. class
#      c1 = call_universalPlotting6b.call_universalPlotting6b(pFile)
#      # call the app. function (0 means the image created is scatter plot)
#      ### (message, imgFileName) = c1.displayScatterPlot2V(0)
#      (message, imgFileName, dataFileName) = c1.display()
#      # chdir back
...
#      purpose = request.args.get('purpose')#"Test .\'\"\\purpose"
#
#      backend_url = 'http://' + hostname + ':' + port + '/svc/universalPlotting6b'
#      print 'backend_url: ', backend_url
#      print 'imgFileName: ', imgFileName
#      plotUrl = 'http://' + hostname + ':' + port + '/static/universalPlotting6b/' + tag + '/' + imgFileName
#      print 'plotUrl: ', plotUrl
#      dataUrl = 'http://' + hostname + ':' + port + '/static/universalPlotting6b/' + tag + '/' + dataFileName
#      print 'dataUrl: ', dataUrl
#      failedImgUrl = 'http://' + hostname + ':' + port + '/static/plottingFailed.png'
#      print 'failedImgUrl: ', failedImgUrl
#
...
'''
    ...
    try:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('scatterPlot2V', tag, imgFileName, dataFileName)
#      hostname1 = "api.jpl-cmda.org"
#      backend_url = 'https://' + hostname1 + '/svc/scatterPlot2V'
#      print 'backend_url: ', backend_url
#      plotUrl = 'https://' + hostname1 + '/static/scatterPlot2V/' + tag + '/' + imgFileName
#      print 'plotUrl: ', plotUrl
#      dataUrl = 'https://' + hostname1 + '/static/scatterPlot2V/' + tag + '/' + dataFileName
#      print 'dataUrl: ', dataUrl
#      failedImgUrl = 'https://' + hostname1 + '/static/plottingFailed.png'
#      print 'failedImgUrl: ', failedImgUrl
...
def displayDiffPlot2V():
    ...
    ferretLevel1 = request.args.get('ferretLevel1', '')
    ferretLevel2 = request.args.get('ferretLevel2', '')
    ferretLevel3 = request.args.get('ferretLevel3', '')

    colorMap1 = request.args.get('colorMap1', '')
    colorMap2 = request.args.get('colorMap2', '')
    colorMap3 = request.args.get('colorMap3', '')
    ...
    json1 = {
       ...
       'lat1S':latS, 
       'lat1E':latE, 
       'ferretLevel1':ferretLevel1, 
       'ferretLevel2':ferretLevel2, 
       'ferretLevel3':ferretLevel3, 
       'colorMap1':colorMap1, 
       'colorMap2':colorMap2, 
       'colorMap3':colorMap3, 
       'nSample':0, 
    ...
    try:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('diffPlot2V', tag, imgFileName, dataFileName)
...
def displayConditionalSamp():
    ...
    try:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('conditionalSampling', tag, imgFileName, dataFileName)
...
def displayConditionalSamp2Var():
    ...
    try:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('conditionalSampling2Var', tag, imgFileName, dataFileName)
...
def regridAndDownload():
    ...
    try:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('regridAndDownload', tag, imgFileName, dataFileName)
...
def multiModelStatistics():
    ...
    try:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('multiModelStatistics', tag, imgFileName, dataFileName)
...
#      print 'notAvailImgUrl: ', notAvailImgUrl
      ...
      #== comment this for not checking imgFileName
      #if imgFileName is '' or not os.path.exists(output_dir+'/'+imgFileName):
      #  print '****** Error: %s not exist' % imgFileName
      #  plotUrl = notAvailImgUrl
...
def displayCorrelationMap():
    ...
    if 1:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('correlationMap', tag, imgFileName, dataFileName)
...
def displayEOF():
    ...
    try:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('EOF', tag, imgFileName, dataFileName)
...
def displayConditionalPdf():
    ...
    try:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('conditionalPdf', tag, imgFileName, dataFileName)
#E      backend_url = 'http://' + hostname + ':' + port + '/svc/conditionalPdf'
#E      print 'backend_url: ', backend_url
#E      print 'imgFileName: ', imgFileName
#E      plotUrl = 'http://' + hostname + ':' + port + '/static/conditionalPdf/' + tag + '/' + imgFileName
#E      print 'plotUrl: ', plotUrl
#E      dataUrl = 'http://' + hostname + ':' + port + '/static/conditionalPdf/' + tag + '/' + dataFileName
#E      print 'dataUrl: ', dataUrl
#E      failedImgUrl = 'http://' + hostname + ':' + port + '/static/plottingFailed.png'
#E      print 'failedImgUrl: ', failedImgUrl
...
def displayAnomaly():
    ...
    try:
      ...
      backend_url, plotUrl, dataUrl, failedImgUrl = assignUrl('anomaly', tag, imgFileName, dataFileName)
...
def displayZonalMean():
    ...
    serviceObj = call_zonalMean.call_zonalMean(request.args.to_dict())
    return generalServiceResp(serviceObj)
...
#== to_workflow_server 
@app.route('/svc/send2Wf', methods=["GET"])
@crossdomain(origin='*')
def send2Wf():
  print 'in send2Wf---------'
  workflowUrl = 'http://hawking.sv.cmu.edu:9069/paramcallback'
  #args1 = request.args.lists()
  ...
  if 1:
    #json1 = jsonify(args1)
    json1 = json.dumps(args1)
    message = requests.post(workflowUrl, data=json1, headers=HEADERS).text
    #message = requests.post(workflowUrl, data=json1, headers=HEADERS)
    print 'message from workflowUrl: '
    print message
    success = True
    return jsonify({
      'success': success,
...
#== wfDisplay__ 
@app.route('/svc/wfDisplay', methods=["GET"])
@crossdomain(origin='*')
def wfDisplay():
  print 'in wfDisplay---------'
...
#== generic_service__
@app.route('/svc/generic_service', methods=["GET"])
@crossdomain(origin='*')
def generic_service():
    """Run generic_service"""
    executionStartTime = int(time.time())
    ...
    try:
      service_name = json2['service_name']
    except:
      service_name = ''
    ...
    #try:
      seed_str = str(time.time())
      tag = md5.new(seed_str).hexdigest()
      output_dir = current_dir + '/svc/static/generic_service/' + serviceName + '_' + tag
      print 'output_dir: ', output_dir
      if not os.path.exists(output_dir):
        os.makedirs(output_dir)
      json2['outDir'] = output_dir
